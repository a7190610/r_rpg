export const RO_DATABASE_SKILLS = {
  ...
  data: {
    ...
    swordman: [
      ...
      ['s_provoke',  '挑釁',         '💢', 'active',  10, 10, (lv) => `怪物防禦力下降 ${lv*5}%，但攻擊力上升 ${lv*2}%`, { spCost: 10, debuffDuration: 15, debuffAttr: (lv) => ({ defPct: -lv*5, atkPct: lv*2 }) } ],
    ...
    merchant: [
      ...
      ['me_vending',    '露天商店',   '🏪', 'active',  10, 15, (lv) => `可開設商店販售物品，最高可賣 ${lv+2} 種商品`, { passiveAttr: (lv) => ({ vendingSlots: lv+2 }) } ],
    ...
    knight: [
      ...
      ['k_cavalry',   '騎乘攻擊',   '🏇', 'passive', 5,  25, (lv) => `恢復騎乘大嘴鳥時下降的攻擊速度 (最高恢復100%)`, { passiveAttr: (lv) => ({ aspdPenaltyReduction: lv*20 }) } ],
    ...
    wizard: [
      ...
      ['w_quagmire',   '泥沼地',   '🕳️', 'active', 5,  15, (lv) => `減少範圍內敵人 AGI 與 DEX 各 ${lv*10}%，並解除加速狀態`, { spCost: 20, debuffDuration: 15, debuffAttr: (lv) => ({ agiPct: -lv*10, dexPct: -lv*10 }), removeBuffs: ['agi'] } ],
      ...
      ['w_sense',      '怪物情報', '👁️', 'active', 1,  5,  (lv) => `消耗 10 SP，獲取目標魔物的血量、屬性與弱點`, { spCost: 10, scanTargetInfo: true } ]
    ],
    assassin: [
      ...
      ['as_venom',     '毒性感染', '☠️', 'active',  5,  20, (lv) => `對已中毒的敵人造成強力爆發毒屬性傷害`, { spCost: 15, dmgMulti: (lv) => 2.0+lv*1.0, hits: 1, element: 'poison', requirePoison: true } ],
      ['as_right',     '右手修練', '🗡️', 'passive', 5,  10, (lv) => `裝備雙刀時，恢復右手因雙持降低的傷害 (最高100%)`, { passiveAttr: (lv) => ({ rightHandPenaltyReduction: lv*20 }) } ],
      ['as_left',      '左手修練', '🗡️', 'passive', 5,  10, (lv) => `裝備雙刀時，恢復左手因雙持降低的傷害 (最高80%)`, { passiveAttr: (lv) => ({ leftHandPenaltyReduction: lv*16 }) } ]
    ],
    priest: [
      ...
      ['pr_res',    '復活術',       '👼', 'active', 4,  20, (lv) => `消耗 60 SP (需藍礦)，復活隊友並恢復 ${lv*20}% HP`, { spCost: 60, reviveHpPct: (lv) => lv*20, requireItem: 'blue_gemstone' } ],
      ['pr_asper',  '灑水祈福',     '💧', 'active', 5,  10, (lv) => `消耗 20 SP (需聖水)，將隊友武器屬性轉為聖屬性`, { spCost: 20, buffAttr: (lv) => ({ element: 'holy' }) } ],
      ['pr_sanct',  '光耀之堂',     '🌟', 'active', 10, 20, (lv) => `消耗 30 SP (需藍礦)，範圍內每秒恢復 777 HP，對不死造成傷害`, { spCost: 30, healPower: 777, healTicks: 10, targetRace: ['undead'], element: 'holy', requireItem: 'blue_gemstone' } ],
      ['pr_lexa',   '轉生術',       '✝️', 'active', 10, 15, (lv) => `機率一擊必殺不死系怪物，失敗則造成聖屬性傷害`, { spCost: 20, instaKillChance: (lv) => lv*5, targetRace: ['undead'], element: 'holy', dmgMulti: (lv) => 1.0+lv*0.2 } ]
    ],
    hunter: [
      ['ht_blitz',      '閃電衝擊', '🦅', 'active',  5,  1,  (lv) => `老鷹造成無視防禦與迴避的 ${lv} 段強制無屬性傷害`, { dmgMulti: 1.0, hits: (lv) => lv, ignoreDef: true, ignoreFlee: true } ],
      ['ht_falcon',     '獵鷹尋笛', '🦉', 'passive', 1,  10, (lv) => `普攻時視 LUK 機率自動觸發閃電衝擊 (需裝備弓與老鷹)`, { passiveAttr: (lv) => ({ autoBlitzChanceBonus: true }) } ],
      ['ht_trap_clay',  '爆炎陷阱', '💥', 'active',  5,  20, (lv) => `消耗 15 SP，設置陷阱造成大範圍火屬性傷害 (視DEX/INT)`, { spCost: 15, dmgMulti: (lv) => 2.0+lv*1.0, element: 'fire' } ],
      ['ht_trap_frost', '霜凍陷阱', '❄️', 'active',  5,  15, (lv) => `造成水屬性傷害並高機率冰凍範圍內的敵人`, { spCost: 15, dmgMulti: 1.5, element: 'water', freezeChance: (lv) => 50+lv*10 } ],
      ['ht_trap_ankle', '定位陷阱', '⛓️', 'active',  5,  10, (lv) => `將踩到的魔物完全定身 ${lv*4} 秒`, { spCost: 10, debuffDuration: (lv) => lv*4, debuffAttr: (lv) => ({ root: true }) } ],
      ['ht_beast',      '野獸剋星', '🐺', 'passive', 10, 5,  (lv) => `對動物系與昆蟲系魔物傷害增加 ${lv*4}%`, { passiveAttr: (lv) => ({ raceDmgBonus: { beast: lv*4, insect: lv*4 } }) } ],
      ['ht_phobia',     '滑動陷阱', '💨', 'active',  5,  10, (lv) => `將踩到的魔物強制向前滑動並受到少量傷害`, { spCost: 10, dmgMulti: 0.5, hits: 1, knockback: true } ]
    ],
    blacksmith: [
      ...
      ['bs_weapon',       '武器研究',     '🔨', 'passive', 10, 10, (lv) => `基礎攻擊力與命中率各提升 ${lv*2}，提高鍛造成功率`, { passiveAttr: (lv) => ({ skAtk: lv*2, hit: lv*2, forgeSuccessPct: lv*2 }) } ],
      ...
      ['bs_hammer',       '大地之擊',     '⛰️', 'active',  5,  25, (lv) => `敲擊地面造成範圍傷害並機率暈眩`, { spCost: 15, dmgMulti: (lv) => 1.0+lv*0.5, hits: 1, stunChance: (lv) => lv*5 } ],
      ['bs_ore',          '尋找礦石',     '💎', 'passive', 1,  10, (lv) => `打怪時有機率額外掉落各種礦石`, { passiveAttr: (lv) => ({ oreDropChance: lv*2 }) } ]
    ],
    ...
    crusader: [
      ...
      ['cr_holy_cross',   '聖十字攻擊', '✝️', 'active',  6,  10, (lv) => `雙手劍專用，造成 ${100+lv*35}% 聖屬性傷害，機率致盲`, { spCost: 15, dmgMulti: (lv) => 1.0+lv*0.35, hits: 1, element: 'holy', blindChance: (lv) => lv*5, weaponReq: 'two_handed_sword' } ],
      ['cr_sacrifice',    '捨命護衛',   '🩸', 'active',  5,  25, (lv) => `與隊友連線，代為承受其受到的所有傷害`, { spCost: 20, buffDuration: (lv) => 30+lv*10, buffAttr: (lv) => ({ takeDamageForAlly: true }) } ],
      ['cr_peco',         '騎乘術(十字軍)', '🦤', 'passive', 1,  10, (lv) => `可騎乘大嘴鳥，增加移動速度與負重`, { passiveAttr: (lv) => ({ moveSpeedPct: 25, weightBonus: 1000 }) } ]
    ],
    sage: [
      ...
      ['sa_volcano', '火元素領域', '🌋', 'active',  5,  20, (lv) => `地上鋪設領域，範圍內火屬性傷害增加，且無法被冰凍`, { spCost: 30, buffDuration: (lv) => 30+lv*10, buffAttr: (lv) => ({ fireDmgPct: 10+lv*5, immuneFreeze: true }) } ],
      ['sa_deluge',  '水元素領域', '🌊', 'active',  5,  20, (lv) => `地上鋪設領域，範圍內水屬性傷害增加，可使用水屬性技能`, { spCost: 30, buffDuration: (lv) => 30+lv*10, buffAttr: (lv) => ({ waterDmgPct: 10+lv*5 }) } ],
      ['sa_whirl',   '風元素領域', '🌪️', 'active',  5,  20, (lv) => `地上鋪設領域，範圍內風屬性傷害增加，FLEE 提升`, { spCost: 30, buffDuration: (lv) => 30+lv*10, buffAttr: (lv) => ({ windDmgPct: 10+lv*5, flee: lv*10 }) } ],
      ['sa_land',    '地元素領域', '🧱', 'active',  5,  25, (lv) => `無效化範圍內所有地面魔法與陷阱 (地領)`, { spCost: 40, removeGroundEffects: true } ]
    ],
    rogue: [
      ...
      ['rg_strip_weap',  '卸除武器',     '🗡️', 'active',  5,  15, (lv) => `機率強制卸除敵方武器，使其攻擊力大幅下降`, { spCost: 15, stripChance: (lv) => lv*5, debuffDuration: 10, debuffAttr: (lv) => ({ atkPct: -25 }) } ],
      ['rg_strip_shld',  '卸除盾牌',     '🛡️', 'active',  5,  15, (lv) => `機率強制卸除敵方盾牌，使其防禦力下降`, { spCost: 15, stripChance: (lv) => lv*5, debuffDuration: 10, debuffAttr: (lv) => ({ defPct: -25 }) } ],
      ['rg_sword',       '單手劍修練(流氓)', '⚔️', 'passive', 10, 10, (lv) => `裝備單手劍時攻擊力增加 ${lv*4}`, { passiveAttr: (lv) => ({ skAtk: lv*4, weaponReq: 'sword' }) } ]
    ],
    monk: [
      ['mo_ashura', '阿修羅霸凰拳', '👊', 'active',  5,  25, (lv) => `消耗所有 SP，造成毀滅性真實傷害，施放後 5 分鐘無法回魔`, { spCost: 999, dmgMulti: (lv) => 8.0+lv*2.0, hits: 1, ignoreDef: true, ignoreFlee: true } ],
      ...
      ['mo_sphere', '蓄氣',         '🔮', 'active',  5,  5,  (lv) => `主動聚集氣球，每顆氣球增加 3 ATK (最高 5 顆)`, { spCost: 8, buildStack: 'sphere', maxStacks: 5, stackAttr: { flatAtk: 3 } } ],
      ['mo_finger', '彈指神功',     '🎯', 'active',  5,  15, (lv) => `消耗所有氣球，遠距離投擲，每顆氣球造成 ${100+lv*50}% 傷害`, { hits: (lv) => lv } ],
      ['mo_combo1', '六合拳',       '🤛', 'passive', 10, 10, (lv) => `普攻時機率自動觸發三連擊，為連技的起手式`, { passiveAttr: (lv) => ({ autoCombo1Chance: lv*2, combo1Dmg: 1.0+lv*0.2 }) } ],
      ['mo_combo2', '連環全身掌',   '🤜', 'active',  5,  15, (lv) => `接在六合拳之後施放，造成 4 段傷害`, { spCost: 10, dmgMulti: (lv) => 2.0+lv*0.5, hits: 4, requireStack: 'combo1' } ],
      ['mo_combo3', '猛龍誇強',     '🐉', 'active',  5,  20, (lv) => `接在連環全身掌之後，造成巨大傷害並擊退`, { spCost: 15, dmgMulti: (lv) => 3.0+lv*1.0, hits: 1, knockback: true, requireStack: 'combo2' } ]
    ],
    bard_dancer: [ 
      ['bd_vulcan',  '奧義箭亂舞',   '🎸', 'active',  10, 1,  (lv) => `裝備樂器/鞭子，發出 9 段共 ${300+lv*100}% 物理傷害`, { dmgMulti: (lv) => (3.0+lv*1.0)/9, hits: 9 } ],
      ['bd_bragi',   '布萊奇之詩',   '🎵', 'active',  10, 10, (lv) => `全隊技能冷卻時間與詠唱減少 (受INT/DEX加成)`, { spCost: 30, buffDuration: 60, buffAttr: (lv) => ({ castTimeReduction: 10+lv*2, cdReduction: 10+lv*2 }) } ],
      ['bd_apple',   '伊登的蘋果',   '🍎', 'active',  10, 15, (lv) => `全隊 MaxHP 提升 ${lv*2}% 並每秒大幅恢復生命`, { spCost: 40, buffDuration: 60, buffAttr: (lv) => ({ maxHpPct: lv*2, hpRegenFlat: lv*50 }) } ],
      ['bd_music',   '樂器/練習曲修練', '🎺', 'passive', 10, 5,  (lv) => `裝備樂器或鞭子時，基礎攻擊力增加 ${lv*3}`, { passiveAttr: (lv) => ({ skAtk: lv*3 }) } ],
      ['bd_joke',    '冷笑話',       '🥶', 'active',  5,  20, (lv) => `全畫面機率性將敵人與隊友冰凍`, { spCost: 20, freezeChance: (lv) => 20+lv*5 } ],
      ['bd_tarot',   '塔羅牌命運',   '🃏', 'active',  5,  25, (lv) => `隨機抽取一張塔羅牌，對敵人造成各種強大異常狀態或傷害`, { spCost: 40, randomStatusEffect: true } ],
      ['bd_lullaby', '搖籃曲',       '💤', 'active',  1,  20, (lv) => `大範圍使敵人陷入睡眠狀態`, { spCost: 30, sleepChance: (lv) => 30+lv*10 } ]
    ],
    alchemist: [
      ...
      ['al_homun',   '生命體呼喚',   '🐥', 'active',  1,  15, (lv) => `召喚生命體協助攻擊，生命體擁有獨立技能與 AI`, { spCost: 50, summon: 'homunculus' } ],
      ...
      ['al_plant',   '召喚植物',     '🌿', 'active',  5,  10, (lv) => `消耗植物瓶，召喚固定砲塔植物協助攻擊`, { spCost: 10, summon: 'plant', requireItem: 'plant_bottle' } ],
      ['al_axe',     '斧頭修練',     '🪓', 'passive', 10, 5,  (lv) => `裝備斧頭時攻擊力增加 ${lv*3}`, { passiveAttr: (lv) => ({ skAtk: lv*3, weaponReq: 'axe' }) } ]
    ],
    ...
    guillotine_cross: [
      ...
      ['gx_hallu',    '幻影幻覺',   '👁️', 'active',  5,  10, (lv) => `降低目標命中與攻速，並造成持續百分比傷害`, { spCost: 40, debuffDuration: 15, debuffAttr: (lv) => ({ hit: -lv*10, aspdBonusPct: -lv*5 }), dotDmgPct: 2 } ]
    ],
    arch_bishop: [
      ...
      ['ab_prae',     '聖母之祈福', '🛡️', 'active',  10, 15, (lv) => `為全隊施加防護盾，抵擋 ${lv*2} 次攻擊`, { spCost: 30, buffDuration: 60, buffAttr: (lv) => ({ shieldHits: lv*2 }) } ],
      ['ab_expi',     '解除',       '🔓', 'active',  5,  5,  (lv) => `賦予隊友無視防禦力 ${lv*5}% 的增益效果`, { spCost: 40, buffDuration: 60, buffAttr: (lv) => ({ ignoreDefPct: lv*5 }) } ],
      ['ab_duple',    '雙向聖光',   '⚖️', 'active',  10, 10, (lv) => `普攻時機率額外觸發聖屬性物理或魔法打擊`, { spCost: 35, buffDuration: 120, buffAttr: (lv) => ({ autoHolyStrikeChance: lv*3 }) } ]
    ],
    ranger: [
      ...
      ['ra_ride',    '狼騎乘',   '🐺', 'passive', 3,  5,  (lv) => `騎乘狼，大幅提升移動速度`, { passiveAttr: (lv) => ({ moveSpeedPct: 20 }) } ],
      ['ra_camo',    '偽裝戰術', '🌿', 'active',  5,  10, (lv) => `隱藏氣息，提升爆擊率與遠程傷害，移動時半透明`, { spCost: 20, buffDuration: 30, buffAttr: (lv) => ({ crit: lv*5, rangeDmgPct: lv*3, sneak: true }) } ],
      ...
      ['ra_breeze',  '微風懼刃', '🍃', 'active',  5,  15, (lv) => `普攻時有機率射出多支箭矢，增加連擊判定`, { spCost: 20, buffDuration: 60, buffAttr: (lv) => ({ multiShotChance: lv*4 }) } ],
    ...
    mechanic: [
      ...
      ['mc_hover',   '懸浮',         '🛸', 'active',  1,  10, (lv) => `機甲懸浮於空中，免疫地面陷阱與部分魔法`, { spCost: 20, buffDuration: 60, buffAttr: (lv) => ({ immuneGround: true }) } ],
      ['mc_shape',   '形體轉換',     '🛡️', 'active',  4,  15, (lv) => `將機甲防禦屬性轉換為 火/水/風/地，降低對應傷害`, { spCost: 30, buffDuration: 120, buffAttr: (lv) => ({ elementResist: lv*5 }) } ],
      ['mc_faw',     '白銀狙擊手',   '🔫', 'active',  5,  10, (lv) => `設置自動攻擊砲塔，協助輸出物理傷害`, { spCost: 40, summon: 'faw_turret' } ]
    ],
    royal_guard: [
      ...
      ['rg_shield',  '盾牌壓擊',   '🛡️', 'active',  5,  5,  (lv) => `單體巨額傷害並高機率使敵人暈眩`, { spCost: 25, dmgMulti: (lv) => 3.0+lv*1.0, hits: 1, stunChance: (lv) => 40+lv*10, weaponReq: 'shield' } ],
      ['rg_inspire', '靈感',       '✨', 'active',  5,  20, (lv) => `解除自身異常，極大提升全素質、攻擊力與最大生命，持續扣除HP`, { spCost: 100, buffDuration: 60, hpDrainPct: 1, buffAttr: (lv) => ({ allStat: lv*5, atkPct: lv*5, maxHpPct: lv*5 }) } ],
      ['rg_banding', '結盟',       '🤝', 'active',  5,  10, (lv) => `與周圍皇家禁衛隊結盟，增加物理攻擊力與防禦力`, { spCost: 40, buffDuration: 60, buffAttr: (lv) => ({ flatAtk: lv*10, def: lv*10 }) } ],
      ['rg_pinpoint','精準攻擊',   '🎯', 'active',  5,  15, (lv) => `瞬間突進並必定爆擊造成傷害，機率破壞敵人裝備`, { spCost: 30, dmgMulti: (lv) => 5.0+lv*1.0, hits: 1, canCrit: true, autoCrit: true, stripChance: (lv) => lv*5 } ]
    ],
    sorcerer: [
      ...
      ['so_fist',    '雙倍投擲(魔法拳)','👊', 'active',  5,  10, (lv) => `將詠唱中的元素箭轉化，接下來數次普攻造成極高魔法爆發傷害`, { spCost: 35, buffDuration: 60, buffAttr: (lv) => ({ magicFistStacks: lv, magicFistDmg: 5.0+lv*1.0 }) } ],
      ['so_warmer',  '極限空虛(暖風)', '♨️', 'active',  5,  15, (lv) => `範圍內解除冰凍/結冰，並每秒恢復隊友 HP`, { spCost: 50, buffDuration: 30, healHpPct: 0.05, cureFreeze: true } ],
      ['so_striking','打擊強化',       '⚔️', 'active',  5,  15, (lv) => `提升自身與隊友的物理攻擊力與暴擊率`, { spCost: 35, buffDuration: 60, buffAttr: (lv) => ({ flatAtk: lv*10, crit: lv*2 }) } ],
      ['so_spirit',  '精靈控制',       '🧚', 'passive', 3,  5,  (lv) => `召喚四大元素精靈，並根據精靈模式獲得強大素質或攻擊加成`, { passiveAttr: (lv) => ({ summonSpirit: true, spiritPower: lv }) } ]
    ],
    shadow_chaser: [
      ...
      ['sc_feint',     '虛擊炸彈', '💣', 'active',  3,  10, (lv) => `留下替身並向後退隱匿，替身爆炸造成大範圍物理傷害`, { spCost: 30, dmgMulti: (lv) => 4.0+lv*1.0, hits: 1, buffAttr: (lv) => ({ sneak: true }) } ],
      ['sc_shadow',    '魅影形態', '🎭', 'active',  5,  10, (lv) => `藏匿於敵方影子中，將自身受到的傷害轉移給該敵人`, { spCost: 40, buffDuration: (lv) => 10+lv*2, buffAttr: (lv) => ({ reflectDmgPct: 100, sneak: true }) } ],
      ['sc_reproduce', '複製',     '📝', 'passive', 10, 5,  (lv) => `能學習其他職業的三轉技能 (上限1招)`, { passiveAttr: (lv) => ({ enableReproduce: true, maxReproduceLv: lv }) } ],
      ['sc_mask_ign',  '無知面具', '🎭', 'active',  3,  15, (lv) => `降低目標 SP，並使其短時間內無法使用技能`, { spCost: 30, debuffDuration: (lv) => 5+lv*2, debuffAttr: (lv) => ({ silence: true, spDrainPct: lv*5 }) } ],
      ['sc_mask_weak', '虛弱面具', '🎭', 'active',  3,  15, (lv) => `解除目標的武器與盾牌，並降低 MaxHP`, { spCost: 30, debuffDuration: (lv) => 5+lv*2, debuffAttr: (lv) => ({ stripWeap: true, stripShield: true, maxHpPct: -lv*3 }) } ],
      ['sc_invis',     '隱形',     '🥷', 'active',  5,  20, (lv) => `進入完全隱形狀態，普攻必定爆擊但持續消耗 SP`, { spCost: 50, spDrainFlat: 10, buffAttr: (lv) => ({ sneak: true, autoCrit: true }) } ]
    ],
    sura: [
      ['su_dragon',    '雙龍腳',     '🐉', 'active', 10, 1,  (lv) => `作為連技起手，造成兩次打擊並使敵人短暫僵直`, { spCost: 15, dmgMulti: (lv) => 1.5+lv*0.3, hits: 2, stunChance: (lv) => 10+lv*2, buildStack: 'dragon_combo' } ],
      ['su_empire',    '大纏崩墜',   '💥', 'active', 5,  5,  (lv) => `接在雙龍腳之後，將敵人定身並造成巨額傷害`, { spCost: 20, dmgMulti: (lv) => 3.0+lv*1.0, hits: 1, debuffDuration: 3, debuffAttr: (lv) => ({ root: true }), requireStack: 'dragon_combo' } ],
      ...
      ['su_cure',      '點穴-活',    '✨', 'active', 5,  10, (lv) => `恢復自身與隊友生命，並解除各種負面狀態`, { spCost: 30, healPower: (lv) => 300+lv*100, cureAll: true } ],
      ['su_rising',    '潛龍昇天',   '🐉', 'active', 10, 20, (lv) => `MaxHP 與 MaxSP 大幅增加，攻速提升，並維持滿氣球狀態`, { spCost: 50, buffDuration: 120, buffAttr: (lv) => ({ maxHpPct: lv*3, maxSpPct: lv*3, aspdBonusPct: 10, maxSpheres: 15 }) } ],
      ['su_lightning', '閃電步',     '⚡', 'active', 5,  10, (lv) => `被遠程攻擊鎖定時瞬間移動至目標身邊反擊`, { spCost: 20, buffDuration: 60, buffAttr: (lv) => ({ autoCounterRange: true }) } ]
    ],
    minstrel_wanderer: [
      ['mw_rain',    '大暴雨',         '🌧️', 'active',  5,  1,  (lv) => `消耗 70 SP，降下箭雨造成 ${lv*12} 段物理傷害 (受AGI/DEX影響)`, { spCost: 70, dmgMulti: 1.0, hits: (lv) => lv*12, weaponReq: 'bow' } ],
      ['mw_metal',   '金屬狂嘯',       '🎸', 'active',  5,  5,  (lv) => `造成巨大單體魔法傷害，同時吸收目標 SP`, { spCost: 40, dmgMulti: (lv) => 5.0+lv*1.0, hits: 1, spDrainPct: 5 } ],
    ...
    genetic: [
      ...
      ['ge_hell',    '地獄植物',       '🥀', 'active',  5,  10, (lv) => `召喚食人植物攻擊周圍敵人，造成無視防禦的固定傷害與出血`, { spCost: 30, flatDmg: (lv) => 1000+lv*500, ignoreDef: true, bleedChance: (lv) => lv*5 } ],
      ['ge_blood',   '吸血植物',       '🩸', 'active',  5,  5,  (lv) => `將吸血種子植入敵人，持續吸取 HP 轉移給施法者`, { spCost: 30, debuffDuration: 15, dotDmgPct: 2, lifestealPct: 100 } ],
      ['ge_spore',   '孢子爆炸',       '🍄', 'active',  5,  10, (lv) => `附著炸彈孢子，數秒後引爆造成廣範圍極高傷害`, { spCost: 40, dmgMulti: (lv) => 5.0+lv*2.0, hits: 1, delay: 3 } ],
      ['ge_howl',    '曼陀羅嚎叫',     '😱', 'active',  5,  15, (lv) => `降低周圍敵人 INT 並增加其固定詠唱時間`, { spCost: 45, debuffDuration: 20, debuffAttr: (lv) => ({ int: -lv*5, fixCastTimeAdd: lv*0.5 }) } ],
      ['ge_research','基因研究',       '🧬', 'passive', 10, 5,  (lv) => `增加藥水恢復量、調配成功率與 MaxSP`, { passiveAttr: (lv) => ({ potionEff: lv*5, maxSpPct: lv*2, craftSuccessPct: lv*2 }) } ]
    ],
    ...
    dragon_knight: [
      ...
      ['dk_servant', '侍從武器',       '🗡️', 'active',  5,  10, (lv) => `召喚虛空幻影武器隨機攻擊鎖定的敵人`, { spCost: 50, summon: 'servant_weapon' } ],
      ['dk_aura',    '龍之光環',       '✨', 'passive', 10, 10, (lv) => `全屬性超幅提升，物理攻擊力極限突破`, { passiveAttr: (lv) => ({ atkPct: lv*2, str: lv*3 }) } ],
      ['dk_vigor',   '活力',           '💪', 'passive', 10, 15, (lv) => `依據自身 MaxHP 大幅增加近戰物理技能的最終傷害`, { passiveAttr: (lv) => ({ hpScalingMeleeBonus: lv*2 }) } ],
      ['dk_breath',  '龍之吐息(四轉)', '🔥', 'active',  10, 20, (lv) => `終極吐息，造成毀滅性範圍傷害，受 Vigor 影響`, { spCost: 80, hpSpScalingDmg: true, hits: 1, element: 'fire', ignoreDef: true } ],
      ['dk_storm',   '風暴斬擊',       '🌪️', 'active',  5,  15, (lv) => `連續兩次十字範圍揮砍，造成極高 AP 爆發傷害`, { spCost: 60, apCost: 10, dmgMulti: (lv) => 15.0+lv*5.0, hits: 2 } ],
      ['dk_pierce',  '衝鋒突刺',       '🔱', 'active',  5,  15, (lv) => `騎乘狀態限定，瞬間突刺造成多段穿甲傷害`, { spCost: 50, dmgMulti: (lv) => 10.0+lv*3.0, hits: 5, ignoreDefPct: 50, requiresMount: true } ]
    ],
    arch_mage: [
      ...
      ['am_slash',     '冰凍斬',           '🧊', 'active',  10, 5,  (lv) => `凝聚冰刃斬擊，造成水魔法傷害並降低敵人魔法防禦`, { spCost: 50, dmgMulti: (lv) => 8.0+lv*2.0, hits: 1, element: 'water', debuffDuration: 10, debuffAttr: (lv) => ({ mdefPct: -lv*5 }) } ],
      ['am_rock',      '岩石崩落',         '⛰️', 'active',  10, 5,  (lv) => `召喚巨石砸擊，對單體及其周圍造成極高地屬性傷害`, { spCost: 60, dmgMulti: (lv) => 12.0+lv*3.0, hits: 1, element: 'earth' } ],
      ['am_astral',    '星辰打擊',         '🌠', 'active',  10, 15, (lv) => `召喚群星隕落，無屬性極致魔法，消耗 AP 爆發`, { spCost: 100, apCost: 20, dmgMulti: (lv) => 30.0+lv*5.0, hits: 5, element: 'neutral' } ],
      ['am_climax',    '巔峰(魔力極限)',   '🔮', 'active',  5,  20, (lv) => `進入巔峰狀態，接下來的四轉魔法產生型態變化與倍率暴增`, { spCost: 150, buffDuration: 60, buffAttr: (lv) => ({ magicSkillDmgMulti: 2.0 }) } ],
      ['am_staff',     '雙手杖修練',       '🪄', 'passive', 10, 10, (lv) => `裝備雙手杖時，無視目標極高比例 MDEF`, { passiveAttr: (lv) => ({ ignoreMdefPct: lv*5, weaponReq: 'two_handed_rod' }) } ],
      ['am_hurricane', '毀滅風暴',         '🌀', 'active',  10, 15, (lv) => `創造毀滅龍捲風，大範圍席捲所有敵人`, { spCost: 90, dmgMulti: (lv) => 15.0+lv*3.0, hits: 5, element: 'wind' } ]
    ],
    shadow_cross: [
      ...
      ['sc_exceed',  '暗影超越',     '🌑', 'active',  5,  5,  (lv) => `短時間內解除攻速上限限制，普攻與技能速度狂飆`, { spCost: 80, buffDuration: 30, buffAttr: (lv) => ({ ignoreAspdLimit: true, aspdBonusPct: 50 }) } ],
      ['sc_knife',   '舞動匕首',     '🔪', 'active',  5,  10, (lv) => `召喚暗影匕首環繞自身，每秒對靠近的敵人造成傷害`, { spCost: 60, buffDuration: 30, dmgMulti: (lv) => 2.0+lv*0.5, hits: 1, interval: 1 } ],
      ['sc_claw',    '致命暗影爪',   '爪', 'active',  10, 15, (lv) => `消耗 AP 的終極斬擊，附帶極高無視防禦與爆擊加成`, { spCost: 100, apCost: 15, dmgMulti: (lv) => 25.0+lv*5.0, hits: 1, ignoreDef: true, canCrit: true, critBonus: 100 } ],
      ['sc_venom',   '強效劇毒',     '🧪', 'passive', 10, 10, (lv) => `提升所有毒屬性技能傷害，新毒效果增強`, { passiveAttr: (lv) => ({ poisonSkillDmgPct: lv*5 }) } ],
      ['sc_eternal', '永恆斬',       '❌', 'active',  10, 5,  (lv) => `十字斬的四轉進化版，連擊次數與傷害翻倍`, { spCost: 50, dmgMulti: (lv) => 24.0+lv*6.0, hits: 4 } ],
      ['sc_sense',   '暗影直覺',     '👁️', 'passive', 10, 5,  (lv) => `永久提升迴避與爆擊傷害，並機率閃避指向技能`, { passiveAttr: (lv) => ({ flee: lv*10, critDmgPct: lv*5, dodgeSkillChance: lv*2 }) } ],
      ['sc_envenom', '劇毒暗影',     '☠️', 'active',  5,  15, (lv) => `將武器浸泡在極致深淵毒液中，普攻附加真實毒傷`, { spCost: 70, buffDuration: 120, buffAttr: (lv) => ({ addPoisonTrueDmg: lv*100 }) } ]
    ],
    cardinal: [
      ['ca_arbitrium',  '仲裁聖光', '⚖️', 'active',  10, 1,  (lv) => `對目標及其周圍造成巨大聖屬魔法傷害，隨後觸發二次爆炸`, { spCost: 90, dmgMulti: (lv) => 20.0+lv*4.0, hits: 2, element: 'holy' } ],
      ['ca_pneumaticus','氣穴風暴', '🌪️', 'active',  10, 5,  (lv) => `無屬性魔法爆發，對聖屬性抗性高的敵人造成毀滅打擊`, { spCost: 80, dmgMulti: (lv) => 15.0+lv*3.0, hits: 1, element: 'neutral', holyResistScaling: true } ],
      ['ca_dilectio',   '神聖治癒', '💖', 'active',  10, 10, (lv) => `消耗 AP，極大範圍全隊究極恢復，並賦予超量護盾`, { spCost: 100, apCost: 20, healPower: (lv) => 3000+lv*500, buffDuration: 20, buffAttr: (lv) => ({ shieldHpPct: 50+lv*5 }) } ],
      ['ca_presens',    '敏銳洞察', '👁️', 'active',  5,  15, (lv) => `提升全隊 CRI 與 爆擊傷害`, { spCost: 60, buffDuration: 120, buffAttr: (lv) => ({ crit: lv*2, critDmgPct: lv*3 }) } ],
      ['ca_argutus',    '銳利武器', '🗡️', 'active',  5,  15, (lv) => `提升全隊 HIT 與 遠程/近戰物理傷害比例`, { spCost: 60, buffDuration: 120, buffAttr: (lv) => ({ hit: lv*3, meleeDmgPct: lv*2, rangeDmgPct: lv*2 }) } ],
      ['ca_fidus',      '堅定信仰', '⛪', 'passive', 10, 5,  (lv) => `聖屬性魔法傷害提升 ${lv*5}%，SP 回復力倍增`, { passiveAttr: (lv) => ({ holyDmgPct: lv*5 }) } ],
      ['ca_benedictum', '祝福',     '👼', 'active',  5,  20, (lv) => `全面提升全隊所有素質 (四轉版天賜)`, { spCost: 100, buffDuration: 180, buffAttr: (lv) => ({ allStat: lv*2 }) } ],
      ['ca_reliquia',   '聖物',     '🛡️', 'active',  5,  20, (lv) => `召喚聖物區域，區域內隊友免疫大部分異常狀態`, { spCost: 120, buffDuration: 30, buffAttr: (lv) => ({ immuneStatus: true }) } ]
    ],
    windhawk: [
      ['wh_gale',     '狂風暴雨(Gale Storm)', '🌪️', 'active',  10, 1,  (lv) => `連續射擊造成大範圍風/無屬性傷害，裝備專用弓傷害提升`, { spCost: 80, dmgMulti: (lv) => 20.0+lv*5.0, hits: 1 } ],
      ['wh_hawk',     '老鷹衝刺',             '🦅', 'active',  10, 5,  (lv) => `指揮四轉獵鷹衝撞，造成極端暴力的單體無視防禦物理傷害`, { spCost: 80, dmgMulti: (lv) => 18.0+lv*4.0, hits: 1, ignoreDef: true } ],
      ['wh_calamity', '災厄狂風',             '🍃', 'active',  5,  15, (lv) => `消耗 AP，進入極限射擊狀態，所有遠程傷害無限制飆升`, { spCost: 100, apCost: 15, buffDuration: 60, buffAttr: (lv) => ({ rangeDmgPct: 50+lv*10 }) } ],
      ['wh_crescive', '新月之箭',             '🏹', 'active',  10, 10, (lv) => `累積能量射出一發穿透極強的箭矢，隨蓄力段數增加倍率`, { spCost: 60, dmgMulti: (lv) => 10.0+lv*3.0, chargeScaling: true, hits: 1, ignoreDefPct: 50 } ],
      ['wh_nature',   '自然親和',             '🍃', 'passive', 10, 5,  (lv) => `遠程物理傷害提升 ${lv*4}%，獵鷹傷害倍增`, { passiveAttr: (lv) => ({ rangeDmgPct: lv*4 }) } ],
      ['wh_trap',     '進階陷阱',             '🕸️', 'active',  5,  10, (lv) => `設置能束縛 Boss 級魔物的強效陷阱，並附帶屬性減抗`, { spCost: 40, debuffDuration: 5, debuffAttr: (lv) => ({ rootBoss: true, allResist: -lv*5 }) } ],
      ['wh_sign',     '風之印記',             '🌀', 'active',  5,  20, (lv) => `標記敵人，使其受到的遠程物理與老鷹傷害暴增`, { spCost: 50, debuffDuration: 15, debuffAttr: (lv) => ({ recvRangeDmgMulti: 1.2+lv*0.05, recvHawkDmgMulti: 1.5+lv*0.1 }) } ],
      ['wh_sharp',    '銳利射擊(強化)',       '🎯', 'passive', 10, 5,  (lv) => `提升爆擊率，使弓箭普攻有機率造成範圍濺射傷害`, { passiveAttr: (lv) => ({ crit: lv*2, splashAttackChance: lv*3 }) } ]
    ],
    meister: [
      ['ms_stomp',   '戰斧踏伐',   '🪓', 'active',  10, 1,  (lv) => `巨斧重擊地面造成 ${2500+lv*400}% 範圍爆發，無視部分防禦`, { spCost: 80, dmgMulti: (lv) => 25.0+lv*4.0, hits: 1, weaponReq: 'axe' } ],
      ['ms_spark',   '火花衝擊',   '💥', 'active',  10, 5,  (lv) => `魔導甲全開，發射高能粒子砲造成無/火屬性連續傷害`, { spCost: 70, dmgMulti: (lv) => 10.0+lv*2.0, hits: 5, element: 'fire', mechaOnly: true } ],
      ['ms_abm',     '愛國者飛彈', '🚀', 'active',  10, 10, (lv) => `鎖定範圍內所有敵人發射導彈，大範圍洗地打擊`, { spCost: 90, dmgMulti: (lv) => 12.0+lv*3.0, hits: 1, mechaOnly: true } ],
      ['ms_rush',    '衝刺地震',   '🏃', 'active',  10, 15, (lv) => `消耗 AP，機甲高速衝撞並引發強烈地震，極度控場`, { spCost: 100, apCost: 15, dmgMulti: (lv) => 20.0+lv*5.0, hits: 1, stunChance: (lv) => 50+lv*5, mechaOnly: true } ],
      ['ms_machine', '製造機甲',   '🤖', 'passive', 5,  5,  (lv) => `允許召喚並搭乘最新型四轉超級魔導甲，解鎖專屬武裝`, { passiveAttr: (lv) => ({ mechSuitV2: true }) } ],
      ['ms_upgrade', '升級套件',   '⚙️', 'passive', 10, 10, (lv) => `提升所有機甲技能傷害 ${lv*3}%，機甲血量上升`, { passiveAttr: (lv) => ({ mechDmgPct: lv*3 }) } ],
      ['ms_mastery', '機甲大師',   '🛠️', 'passive', 10, 5,  (lv) => `雙手斧與機甲武器基礎攻擊力與防禦穿透力大幅提高`, { passiveAttr: (lv) => ({ skAtk: lv*15, ignoreDefPct: lv*3, weaponReq: ['axe', 'mace'] }) } ],
      ['ms_vulcan',  '火神砲升級', '🔥', 'passive', 5,  15, (lv) => `使火神砲成為範圍攻擊，並附帶燃燒狀態`, { passiveAttr: (lv) => ({ vulcanAoe: true, vulcanBurnChance: lv*5 }) } ]
    ],
    imperial_guard: [
      ...
      ['ig_overslash', '超越斬',   '⚔️', 'active',  10, 5,  (lv) => `持盾與槍瞬間進行霸體突破斬擊，爆發極高單體傷害`, { spCost: 60, dmgMulti: (lv) => 15.0+lv*4.0, hits: 1, buffAttr: (lv) => ({ antiFlinch: true }) } ],
      ...
      ['ig_shield',    '神聖護盾', '🛡️', 'active',  5,  10, (lv) => `張開神聖領域，大幅降低全隊受到的所有物理/魔法傷害`, { spCost: 80, buffDuration: 30, buffAttr: (lv) => ({ dmgReductionPct: lv*4 }) } ],
      ['ig_mastery',   '槍劍精通', '🔱', 'passive', 10, 5,  (lv) => `同時裝備單手槍與盾牌時，攻擊力與爆擊率翻倍`, { passiveAttr: (lv) => ({ skAtkMulti: 2.0, critMulti: 2.0, requireSpearAndShield: true }) } ],
      ['ig_aura',      '帝國光環', '✨', 'active',  5,  20, (lv) => `提升自身與周圍隊友的最終傷害減免，並增加聖屬性攻擊力`, { spCost: 60, buffDuration: 60, buffAttr: (lv) => ({ finalDmgReduction: lv*3, holyDmgPct: lv*5 }) } ],
      ['ig_stance',    '守衛姿態', '🛡️', 'active',  5,  10, (lv) => `進入防禦姿態，移動速度下降但獲得無敵格擋機率`, { spCost: 40, buffDuration: 60, buffAttr: (lv) => ({ moveSpeedPct: -30, invulBlockChance: lv*5 }) } ],
      ['ig_shoot',     '盾牌射擊', '🎯', 'active',  10, 5,  (lv) => `投擲出帶有聖光之力的盾牌，遠距離強力打擊`, { spCost: 50, dmgMulti: (lv) => 10.0+lv*2.0, hits: 1, element: 'holy', weaponReq: 'shield' } ]
    ],
    elemental_master: [
      ...
      ['em_spirit',    '元素精靈大師', '🧚', 'passive', 5,  10, (lv) => `召喚四轉精靈王，精靈王擁有獨立高倍率 AI 攻擊技能`, { passiveAttr: (lv) => ({ summonSpiritKing: true, spiritKingPower: lv }) } ],
      ['em_burst',     '元素爆發',     '💥', 'active',  5,  20, (lv) => `獻祭精靈王，瞬間對全畫面造成破萬倍率真實魔法傷害`, { spCost: 150, dmgMulti: (lv) => 100.0+lv*20.0, hits: 1, ignoreMdef: true, requireSpiritKing: true } ]
    ],
    abyss_chaser: [
      ...
      ['ac_strike',  '深淵打擊',     '💥', 'active',  10, 15, (lv) => `消耗 AP 釋放深淵炸彈，造成擴散性魔法與物理混合傷害`, { spCost: 90, apCost: 10, dmgMulti: (lv) => 25.0+lv*5.0, hits: 1, element: 'shadow' } ],
      ['ac_smash',   '暗影粉碎',     '🌑', 'active',  5,  20, (lv) => `卸除並破壞敵方身上所有增益狀態與裝備`, { spCost: 60, stripChance: (lv) => 30+lv*5, removeBuffs: 'all' } ],
      ['ac_chain',   '連鎖反應',     '⛓️', 'passive', 5,  10, (lv) => `使用深淵技能時，高機率無消耗自動追加一次同樣技能`, { passiveAttr: (lv) => ({ autoCastAbyssSkillChance: lv*3 }) } ]
    ],
    inquisitor: [
      ['iq_oleum',     '聖油洗禮',   '🏺', 'active',  10, 1,  (lv) => `潑灑聖油，造成物理傷害並使敵人受到的火/聖屬性傷害翻倍`, { spCost: 80, dmgMulti: (lv) => 5.0+lv*2.0, hits: 5 } ],
      ['iq_explosion', '爆炸爆破',   '💥', 'active',  10, 5,  (lv) => `在浸滿聖油的敵人身上點火，引發毀滅性連鎖爆發`, { spCost: 70, dmgMulti: (lv) => 20.0+lv*4.0, hits: 1, element: 'fire', requireOil: true } ],
      ['iq_massive',   '巨型火爆',   '🔥', 'active',  10, 10, (lv) => `虎砲進化版，根據自身 MaxHP 與失去血量，對大範圍造成真傷`, { spCost: 100, hpCostPct: 0.3, hpSpScalingDmg: true, hits: 1, ignoreDef: true } ],
    ...
    troubadour_trouvere: [
      ...
      ['tt_fury',     '金屬狂怒',   '🎸', 'active',  10, 10, (lv) => `金屬狂嘯進化，音波造成極大範圍無屬性魔法爆發`, { spCost: 80, dmgMulti: (lv) => 20.0+lv*4.0, hits: 1, element: 'neutral' } ],
      ['tt_symphony', '神秘交響曲', '🎼', 'active',  5,  15, (lv) => `消耗 AP 演奏終極交響，使全畫面敵人攻防歸零，隊友無敵 3 秒`, { spCost: 100, apCost: 20, debuffDuration: 10, debuffAttr: (lv) => ({ defPct: -100, mdefPct: -100 }), buffDuration: 3, buffAttr: (lv) => ({ invulnerable: true }) } ],
      ['tt_blend',    '混合音效',   '🎛️', 'active',  5,  10, (lv) => `可將兩首三轉合唱曲的效果獨立釋放，無需另一半職業配合`, { passiveAttr: (lv) => ({ soloEnsemble: true }) } ],
      ['tt_stage',    '舞台禮儀',   '🎭', 'passive', 10, 5,  (lv) => `技能 SP 消耗減少 ${lv*2}%，演奏時免疫斷詠唱與大部分異常`, { passiveAttr: (lv) => ({ spCostReduction: lv*2 }) } ],
      ['tt_encore',   '安可',       '👏', 'active',  1,  20, (lv) => `瞬間重置所有四轉技能的冷卻時間`, { spCost: 150, resetCooldowns: 'job4' } ],
      ['tt_mastery',  '吟遊大師',   '🎻', 'passive', 10, 5,  (lv) => `樂器與鞭子攻擊力增加，音符類魔法傷害獲得 INT/DEX 雙加成`, { passiveAttr: (lv) => ({ skAtk: lv*10, magicScalingStats: ['int', 'dex'], weaponReq: ['instrument', 'whip'] }) } ]
    ],
    biolo: [
      ['bi_pharmacy',  '生化製藥',     '🧪', 'active',  5,  5,  (lv) => `能瞬間大量製造最高階的恢復藥水與生化爆炸瓶`, { spCost: 100, craftHighTier: true, craftAmount: (lv) => lv*2 } ],
      ['bi_creeper',   '爆炸藤蔓',     '🌿', 'active',  10, 1,  (lv) => `召喚藤蔓纏繞敵人並引爆，造成 ${2000+lv*300}% 物理爆發`, { spCost: 75, dmgMulti: (lv) => 20.0+lv*3.0, hits: 1, element: 'earth' } ],
      ['bi_acid_water','強酸領域-水',  '💧', 'active',  10, 5,  (lv) => `消耗四轉強酸瓶，佈置大範圍水屬性強酸，使敵人裝備溶毀`, { spCost: 80, dmgMulti: (lv) => 10.0+lv*2.0, hits: 3, element: 'water', stripChance: (lv) => 10+lv*2, requireItem: 'acid_bottle_v2' } ],
      ['bi_acid_wind', '強酸領域-風',  '🌪️', 'active',  10, 5,  (lv) => `消耗四轉強酸瓶，佈置大範圍風屬性強酸，附帶連續僵直雷擊`, { spCost: 80, dmgMulti: (lv) => 10.0+lv*2.0, hits: 3, element: 'wind', stunChance: (lv) => 20+lv*2, requireItem: 'acid_bottle_v2' } ],
      ['bi_report',    '研究報告',     '📜', 'passive', 10, 10, (lv) => `提升四轉強酸瓶與爆炸藤蔓的最終傷害，增加 INT/STR`, { passiveAttr: (lv) => ({ skillDmgBonus: { bi_creeper: lv*5, bi_acid_water: lv*5, bi_acid_wind: lv*5 }, int: lv*3, str: lv*3 }) } ],
      ['bi_mutant',    '突變植物',     '🥀', 'active',  5,  15, (lv) => `召喚突變巨花王，巨花擁有超高血量並會自動吞噬周圍怪物`, { spCost: 100, summon: 'mutant_flower' } ],
      ['bi_armor',     '生化裝甲',     '🛡️', 'active',  5,  10, (lv) => `用藤蔓將自身包覆，獲得巨額護盾並反彈近戰物理傷害`, { spCost: 70, buffDuration: 60, buffAttr: (lv) => ({ shieldHpPct: 20+lv*5, reflectMeleeDmgPct: lv*5 }) } ],
      ['bi_genetic',   '基因改造',     '🧬', 'passive', 5,  20, (lv) => `生命體全能力解放，攻防血量翻倍，並使其攻擊附帶吸血效果`, { passiveAttr: (lv) => ({ homunStatMulti: 2.0, homunLifestealPct: lv*5 }) } ]
    ]
  }
};
